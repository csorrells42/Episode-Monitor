...
function Get-ReviewFrames {
    ...
    $candidates = @(
        ...
        [pscustomobject]@{ Label = "farthest/smallest face lock"; Record = (Select-FirstSorted $Rows "FaceHeight") },
        [pscustomobject]@{ Label = "closest/largest face lock"; Record = (Select-FirstSorted $Rows "FaceHeight" -Descending) },
...
function Write-HtmlReport {
    ...
    [void]$html.AppendLine("<table><tr><th>Clip</th><th>Face</th><th>Reliability</th><th>Eye</th><th>Mouth</th><th>Quality</th><th>Capture quality</th><th>Corpus</th><th>Identity</th><th>Capture plan</th><th>Artifacts</th><th>Report</th></tr>")
    ...
    foreach ($clip in $ClipResults) {
        ...
        [void]$html.AppendLine("<td>$(Format-Percent $clip.FaceDetectionRate)<br><span class=""small"">x range $(Format-Number $clip.FaceCenterXRange), y range $(Format-Number $clip.FaceCenterYRange), scale $(Format-Number $clip.FaceHeightRange)</span></td>")
        ...
        $eyeInsetClipSummary = if ($eyeInsetEnabled) { "<br><span class=""small"">inset measure $(Format-Percent $clip.EyeInsetMeasurementRate), diagnostics $(Format-Percent $clip.EyeInsetImageQualityRate), agreement $(Format-Number $clip.EyeInsetFullFrameAgreementTrustPercent ""0.0"")%, min open $(Format-Number $clip.EyeInsetMinimumOpening), closure $(Format-Number $clip.EyeInsetCueMaximumClosure), score $(Format-Number $clip.EyeInsetCueMaximumScore ""0.0"")</span>" } else { "" }
        ...
        $corpusLink = if ([string]::IsNullOrWhiteSpace($relativeCorpus)) { "" } else { "<br><a href=""$(HtmlEncode $relativeCorpus)"">readiness</a>" }
        [void]$html.AppendLine("<td>ready $(Format-Number $clip.CorpusReadinessPercent ""0.0"")%<br><span class=""small"">stability $(Format-Number $clip.LearningStabilityCoveragePercent ""0.0"")%, shape $(Format-Number $clip.PersonalModelContourShapeCoveragePercent ""0.0"")%, trust $(Format-Number $clip.PersonalModelDirectFeatureMeasurementTrustPercent ""0.0"")%, anchor $(Format-Number $clip.PersonalModelLearningAnchorPercent ""0.0"")%, next sample $(Format-Number $clip.PersonalModelMaxNextSampleInfluencePercent ""0.0"")%</span><br><span class=""small"">eye trust $(Format-Number $clip.PersonalModelEyeBehindGlassesTrustPercent ""0.0"")%, mouth/jaw $(Format-Number $clip.PersonalModelMouthJawTrustPercent ""0.0"")%, motion pairs $($clip.PersonalFaceMotionModelMotionPairs)</span>$corpusLink</td>")
        [void]$html.AppendLine("<td>coverage $(Format-Number $clip.PersonalModelIdentityCoveragePercent ""0.0"")%<br><span class=""small"">signature $($clip.PersonalModelIdentitySignatureSamples), shape L/R eye $($clip.PersonalFaceCorpusLeftEyeShapeSamples)/$($clip.PersonalFaceCorpusRightEyeShapeSamples), lip $($clip.PersonalFaceCorpusInnerLipShapeSamples), jaw $($clip.PersonalFaceCorpusJawShapeSamples), mismatch rejects $($clip.PersonalModelSubjectMismatchRejectedSamples)</span></td>")
    ...
    foreach ($clip in $ClipResults) {
        ...
        if ($clip.CorpusReadinessNextCaptureSuggestions.Count -gt 0) {
            [void]$html.AppendLine("<h3>Corpus readiness suggestions</h3>")
            [void]$html.AppendLine("<ul>")
