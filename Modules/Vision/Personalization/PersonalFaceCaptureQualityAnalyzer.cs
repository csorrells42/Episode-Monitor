...
public sealed class PersonalFaceCaptureQualityAnalyzer
{
    ...
    public PersonalFaceCaptureQualityAssessment Analyze(PersonalFaceCaptureQualityInput input)
    {
        ...
        var subjectGateOpen = input.PersonalModelUpdate.RejectionKind != PersonalFaceModelRejectionKind.SubjectNotConfirmed;
        var modelAccepting = input.PersonalModelUpdate.Accepted;
        if (!subjectGateOpen)
        {
        ...
        {
            issues.Add($"personal model paused: {input.PersonalModelUpdate.RejectionKind}");
            if (!string.IsNullOrWhiteSpace(input.PersonalModelUpdate.Reason))
            {
                suggestions.Add(input.PersonalModelUpdate.Reason);
            }
        }
    ...
    private static double CalculateCameraModeScore(
        PersonalFaceCaptureQualityInput input,
        ...
        ICollection<string> suggestions)
    {
        ...
        if (score < 58d)
        {
            ...
            suggestions.Add("Collect a quick lighting/glare check before relying on these measurements for avatar learning.");
        }
    ...
    private static double CalculateStorageScore(
        ...
        ICollection<string> suggestions)
    {
        var budget = Math.Max(1L, input.MeasurementBudgetBytes);
        var used = Math.Max(0L, input.MeasurementJournalBytes);
        var usedPercent = used / (double)budget * 100d;
        if (usedPercent > 90d)
        {
            issues.Add($"measurement budget is nearly full: {usedPercent:0}%");
            suggestions.Add("Archive or move old output before collecting more measurements.");
            return 18d;
        }

        if (usedPercent > 75d)
        {
            issues.Add($"measurement budget is getting full: {usedPercent:0}%");
            suggestions.Add("Keep the output folder on the external drive and watch storage before long sessions.");
            return 62d;
        }
    ...
    private static PersonalFaceCaptureQualityAssessment BuildAssessment(
        string label,
        double scorePercent,
        bool canCollect,
        bool avatarReady,
