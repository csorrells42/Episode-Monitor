...
public sealed class PersonalFaceCorpusReadinessBuilder
{
        ...
        PersonalFaceMotionModel motionModel)
    {
        ...
        if (model.PoseBucketConsistency.ComparedPoseBucketCount > 0 && model.PoseBucketConsistencyHealthPercent < 70d)
        {
            ...
            model.NextCaptureSuggestions.Add("Open Face Preview and compare turned-head pose buckets against the neutral face; if eyes, nose, or mouth slide across the head, rebuild avatar data after fixing tracking.");
        }
        ...
        if (model.PoseExplainedFeatureMotionHealthPercent < 70d)
        {
            ...
            model.NextCaptureSuggestions.Add("Review a turned-head overlay or face preview. If facial features slide sideways instead of the head rotating, collect new data only after tracking is corrected.");
        }
        ...
        if (model.MouthVerticalAnchorHealthPercent < 70d)
        {
            ...
            model.NextCaptureSuggestions.Add("Review the overlay while opening/closing the mouth; the lip outline should stay below the nose and centered on the actual lips before avatar learning continues.");
        }
        ...
        if (model.IdentitySessionHealthPercent is > 0d and < 70d)
        {
            ...
            model.NextCaptureSuggestions.Add("Review the collection audit and overlay; keep avatar learning off if someone else is at the camera or the face geometry looks inconsistent.");
        }
        ...
        if (model.SurfaceGeometryPatchCount > 0 && model.SurfaceGeometryHealthPercent < 55d)
        {
            ...
            model.NextCaptureSuggestions.Add("Open the measurement face preview and inspect Surface Patch Geometry; collect stable subject-confirmed frames for any patch marked review, especially thin mouth openings, nose bridge, cheeks, and brows.");
        }
