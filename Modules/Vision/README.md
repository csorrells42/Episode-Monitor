...
- `HeadPoseCalibration.cs`, `HeadPoseEstimate.cs`, `HeadPoseEstimator.cs`, `HeadPoseEstimatorInput.cs`: XYZABC pose and apparent-scale estimator. It reports X/Y frame center, Z apparent camera-space depth, A/B/C rotations, camera-FOV-assisted scale, learned-reference Z ratios once the personal model has enough subject-confirmed face-width and eye-spacing evidence, and Z confidence/source labels so calibrated, camera-FOV, learned-reference, and apparent-only estimates can be audited separately.
...
- `PersonalFaceLearningStability.cs`: JSON-friendly slow-learning policy and stability summary, including accepted weight, weakest tracked distribution weight, anchor status, and conservative maximum next-sample influence so avatar consumers do not treat one new session as a rewrite.
- `PersonalFaceContourShapeProfile.cs`: JSON-friendly aggregate shape profile for fixed resampled eye/lip/jaw contours and brow/nose/cheek/forehead surface profiles. It stores weighted face-local X/Y/Z point distributions only, not per-frame raw contours or meshes.
- `PersonalFacePoseBucketProfile.cs`: JSON-friendly straight-on and A/B/C rotation pose buckets. These keep neutral identity proportions separate from turned-head evidence while still storing compact aggregate measurements only.
- `PersonalFacePoseBucketConsistency.cs`: compares turned-head pose buckets against the front-neutral identity bucket and checks each bucket's claimed pose axis, so an A/B/C bucket with little matching head pose or sliding eye/mouth proportions is flagged before avatar learning trusts it.
...
- `PersonalFaceModel.cs`: durable model object for personal face position, scale, head pose, eye opening, mouth opening, jaw droop, MediaPipe blendshape baselines, aggregate eye/lip/jaw shape profiles, aggregate brow/nose/cheek/forehead surface profiles, pose bucket profiles, quality diagnostics, reconstruction counts, accepted/rejected sample counts, and the slow-learning stability block.
- `PersonalFaceModelBuilder.cs`: conservative online updater that accepts only high-reliability, high-quality, non-event-like frames and rejects no-face, low-quality, and symptom-like samples with an auditable reason. Live and evaluator callers run the capture-quality preflight before allowing this builder to learn from a frame. The builder updates aggregate shape profiles only from direct observations, so reconstructed or artifact-suppressed eye/lip contours do not become shape truth. Dense mesh frames also update brow, nose bridge/base, cheek, and forehead surface profiles with face-local Z evidence. Accepted frames also update explicit Z distributions and source counters from the head-pose estimator, then update pose buckets so front-neutral identity, left/right B turns, up/down A tilt, and C-axis tilt can be audited separately. Its learning-stability report bounds next-sample influence using the weakest existing face/feature/shape distribution, not just the global accepted-sample weight.
...
- `PersonalFaceApertureConsistency.cs`: compares recent eye, mouth, and jaw opening measurements against dense blink/jaw/mouth corroboration so behind-glasses eyelid data or bad lip locks can be caught before avatar fitting.
- `PersonalFaceCorpusReadiness.cs`: durable readiness/report JSON object for extreme-accuracy learning-data coverage, learning-stability anchoring, Z distance plus A/B/C rotation coverage, explicit Z evidence health, pose bucket coverage, pose-bucket consistency, recent identity-session health plus warmup/mature stage, aperture consistency, eye aperture reliability, pose-explained feature motion, mouth vertical-anchor sanity, aggregate contour-shape coverage, contour Z profile health, aggregate surface-shape coverage, surface Z profile health, surface-geometry health/status, direct eye-behind-glasses and mouth/jaw trust, capture-quality coverage, storage health, warnings, and next capture suggestions.
- `PersonalFaceCorpusReadinessBuilder.cs`: scores baseline sample count, slow-learning anchor strength, motion pairs, face-size/distance range, named Z/A/B/C coverage, head-pose range, explicit pose bucket coverage, pose-bucket identity and axis consistency, recent identity-session confidence/outlier drift, aperture corroboration, eye aperture reliability, pose-explained feature motion, mouth vertical-anchor health, eye/mouth/jaw expression range, aggregate eye/lip/jaw contour-shape profiles, aggregate contour Z profile health, aggregate brow/nose/cheek/forehead surface profiles, aggregate surface Z profile health, surface patch geometry coherence, direct feature trust, measurement quality, capture-quality gate pass rate, avatar-grade rate, and storage headroom. The eye audit flags one-eye glasses/contour artifacts, suppressed eye evidence, reconstructed eye-side measurements, and weak left/right eye agreement before eyelid data can quietly pollute narcolepsy or avatar fitting. The identity-session audit separates sample/baseline warmup from mature review/hold states, then flags a recent accepted stretch whose measurement-only subject signature no longer agrees with the learned person. The pose-explained feature audit compares observed eye/mouth/spacing drift with measured A/B/C rotation so head turns do not become face-shape rewrites. The pose-bucket axis audit flags an A/B/C bucket whose averaged head pose does not match the bucket label. The surface-geometry audit flags folded, thin, uneven, or degenerate learned patches before they can quietly pollute avatar fitting. The mouth-anchor audit flags lip measurements that sit too high on the face or appear locked on the area under the nose before they can quietly pollute avatar fitting.
- `PersonalFaceCorpusReadinessStore.cs`: JSON and dark HTML writer for `personal_face_corpus_readiness.json` and `personal_face_corpus_readiness.html` in live app, evaluator, and synthetic stress outputs, including pose bucket, pose-bucket consistency, identity-session, aperture consistency, eye aperture reliability, pose-explained feature motion, mouth-anchor health, contour-shape, contour Z profile, surface-shape, surface Z profile, surface-geometry health, direct feature-trust, and capture-quality score bars plus issue rollups.
- `PersonalFaceLearningAuditGate.cs`: reusable consistency gate that pauses new long-term avatar learning only after enough samples exist and high-risk tracking findings show identity-session drift, pose axes, face-local features, or surface geometry may be drifting into bad data.
...
- `PersonalFaceCollectionAudit.cs`: durable collection audit/report JSON object explaining why attempted frames were or were not learned, including face-lock rate, subject-gate rate, accepted rate, collectable/avatar-grade rates, tracking-audit hold counts, rejection counts, quality subscores, top issues, and next actions.
...
- `MeasurementFacePreviewSurfaceEvidence.cs`: JSON-friendly surface-confidence row for eyes, brows/forehead, nose projection, cheeks, lips, and jaw/chin. It separates front-facing evidence from side/depth evidence and includes Z profile coverage/stability/range so the preview can show what is measured versus still inferred.
...
- `MeasurementAvatarCapturePlanBuilder.cs`: turns readiness gaps into concrete next-session tasks for baseline, distance, pose, expression, identity, contour-shape, behind-glasses eye trust, eye aperture reliability, mouth/jaw trust, quality, capture-quality, motion, and storage coverage.
...
- `MeasurementAvatarEasyModeInput.cs`, `MeasurementAvatarEasyModeState.cs`, `MeasurementAvatarEasyModeAdvisor.cs`: testable easy-mode decision layer that turns the current subject/camera/capture-quality state and capture plan into one next action for guided avatar collection.
- `LastGoodFeatureMeshSampleFactory.cs`, `LastGoodFeatureMeshWireframeBuilder.cs`, `LastGoodFeatureMeshStabilityAnalyzer.cs`, `LastGoodFeatureMeshStore.cs`: retain the last 10 subject-confirmed dense mesh frames that have direct high-quality eye and mouth feature lock, build a conservative curated facial scaffold plus highlighted facial feature edges, score head-locked feature-center drift, and write the dark interactive `last_10_good_features` JSON/HTML audit view. The retained samples now store the current pose-estimator XYZABC/Z values and source labels, so the last-good page uses the same A/B/C and apparent-depth evidence as the live avatar system. The A/B/C/Z stability scores use axis-relevant samples plus true neutral reference samples and expose compared-feature counts, so an A tilt problem does not falsely dirty the B head-turn score. The viewer can switch between camera-bounds and head-locked coordinates so ghosted samples show whether eyes, brows, nose, forehead, cheeks, lips, and jaw stay anchored to the head instead of sliding across it.
