...
public sealed class HeadPoseEstimator
{
    ...
    public HeadPoseEstimate Estimate(HeadPoseEstimatorInput input)
    {
        ...
        if (!frame.HasFace)
        {
            return HeadPoseEstimate.None;
        }

        var pose = EstimatePoseDegrees(frame);
        ...
        var status = FormatStatus(pose, distance, apparentDistance.Units, relativeScale.Scale, faceBounds, eyeSpan, confidence, zConfidence);

        return new HeadPoseEstimate
        {
            HasFace = true,
    ...
    private static (double YawDegrees, double PitchDegrees, double RollDegrees, string Source) EstimatePoseDegrees(
        ...
        double? interEyeFrameWidth)
    {
        ...
        var calibration = input.Calibration ?? HeadPoseCalibration.None;
        ...
        double zConfidence)
    {
        ...
        var fillLabel = faceBounds is { } bounds
            ...
            : "";
        return $"Pose: {distanceLabel}{relativeLabel}{fillLabel} | A around X {pose.PitchDegrees:0.#} deg | B around Y {pose.YawDegrees:0.#} deg | C around Z {pose.RollDegrees:0.#} deg | pose q {confidence:0}% | Z q {zConfidence:0}%";
    }
